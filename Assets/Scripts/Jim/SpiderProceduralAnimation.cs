...
public class SpiderProceduralAnimation : MonoBehaviour
{
    ...
    Vector3[] MatchToSurfaceFromAbove(Vector3 point, float halfRange, Vector3 up)
    {
        ...
        if (Physics.SphereCast(ray, sphereCastRadius, out hit, 2f * halfRange, mask))
        {
            res[0] = hit.point;
            res[1] = hit.normal;
        }
    ...
    void Start()
    {
        mask = LayerMask.GetMask("IgnorePlayer", "Jim");
        speed = spider._speed;
    ...
    void FixedUpdate()
    {
        ...
        velocity = ((velocity / speed) + smoothness * lastVelocity) / (smoothness + 1f);
        ...
        if (nbLegs > 3 && bodyOrientation)
        {
            ...
            Vector3 v2 = legTargets[1].position - legTargets[3].position;
