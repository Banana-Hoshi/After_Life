...
public class SpiderController : MonoBehaviour
{
    ...
    public JimScare scare;
    public EventReference jimDeath;
    ...
    Vector3[] GetClosestPointIco(Vector3 point, Vector3 up, float halfRange)
    {
        ...
        foreach (Vector3 dir in dirs)
        {
            ...
            if (Physics.SphereCast(ray, 0.01f, out hit, 2f * halfRange, mask))
            {
                res[0] += hit.point;
                res[1] += hit.normal;
                amount += 1;
    ...
    static Vector3[] GetClosestPoint(Vector3 point, Vector3 forward, Vector3 up, float halfRange, float eccentricity, float offset1, float offset2, int rayAmount, LayerMask _mask)
    {
        Vector3[] res = new Vector3[2] { point, up };
        ...
        foreach (Vector3 dir in dirs)
        {
            ...
            //Debug.DrawRay(ray.origin, ray.direction);
            if (Physics.SphereCast(ray, 0.01f, out hit, 2f * halfRange, _mask))
            {
                res[0] += hit.point;
                res[1] += hit.normal;
                normalAmount += 1;
    ...
    void Start()
    {
        mask = LayerMask.GetMask("IgnorePlayer");
        scare = FindObjectOfType<JimScare>().GetComponent<JimScare>();
        body = GetComponent<Rigidbody>();
        if (player == null)
        {
            player = FindObjectOfType<PlayerController>().transform;
            originalPlayer = FindObjectOfType<PlayerController>().transform;
        }
    ...
    void FixedUpdate()
    {
        ...
        //    multiplier = 2f;
        if (transform.position.y != player.position.y && transform.position.z != player.position.z)
            transform.position += transform.forward * 1f * _speed * multiplier * Time.fixedDeltaTime;
        //if (transform.position.x != player.position.x)
            //transform.position += Vector3.Cross(transform.up, transform.forward) * 1f * _speed * multiplier * Time.fixedDeltaTime;

        transform.position = Vector3.MoveTowards(transform.position, player.position, Time.fixedDeltaTime * _speed * multiplier);

            if (velocity.x != 0 || velocity.y != 0 || velocity.z != 0)
        {
            pn = GetClosestPoint(transform.position, transform.forward, transform.up, 0.5f, 0.1f, 30, -30, 4, mask);
                    pn = GetClosestPointIco(transform.position, transform.up, 0.2f);

            upward = pn[1];

            Vector3[] pos = GetClosestPoint(transform.position, transform.forward, transform.up, 0.5f, raysEccentricity, innerRaysOffset, outerRaysOffset, raysNb, mask);
    ...
    {
        RuntimeManager.PlayOneShotAttached(jimDeath, gameObject);
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            scare.KillAll();
            scare.JimJumpScare(gameObject.transform, Quaternion.identity);
            Destroy(gameObject);
        }
    }
}
